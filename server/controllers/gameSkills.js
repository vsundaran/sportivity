...
exports.getSkills = async (req, res) => {
  try {
    ...
    if (!userdata) {
      return res
        .status(404)
        .json({ success: false, message: "User skills not found" });
    }
    ...
    if (!data) {
      return res
        ...
        .json({
          success: false,
          message: "Game skills not found for the primary sport",
        });
    }
    res.json({
      success: true,
      message: "Game skills fetched successfully",
      data,
    });
  } catch (err) {
    res
      ...
      .json({
        success: false,
        message: "Failed to fetch game skills",
        error: err.message,
      });
  }
};
